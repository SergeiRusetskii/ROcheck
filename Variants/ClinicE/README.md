...
- Configuration-driven thresholds and exclusions in `Config/ClinicEConfig.cs`
- Composite validator architecture with seven validation components
...
- reviewed prescriptions drive target filtering
...
- `Info` when no reviewed prescriptions are available and target checks are skipped
...
- ClinicE uses a configurable 4 mm threshold
...
- ClinicE uses configuration-driven thresholds:
...
- ClinicE uses a configurable SIB threshold of `>6%`
...
Validators are composed through a shared root validator, with ClinicE injecting a configuration object for clinic-specific thresholds and exclusions.
...
- Windows x64
- Varian Eclipse 18.0 with ESAPI access
- .NET Framework 4.8
