...
namespace ROcheck
{
    ...
    // 2.9 Clinical goals and target structure validation
    public class ClinicalGoalsValidator : ValidatorBase
    {
        private static readonly HashSet<string> ExcludedBones = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
        {
            "Bone", "Bones", "Femur_L", "Femur_R", "Mandible", "Pelvis", "Spine"
        };
        ...
        public override IEnumerable<ValidationResult> Validate(ScriptContext context)
        {
            ...
            ValidateTargetContainment(structureSet.Structures, results);
            ValidatePtvsOverlappingOars(structureSet.Structures, structureGoals, results);
        ...
        private static void ValidateTargetContainment(IEnumerable<Structure> structures, List<ValidationResult> results)
        {
            ...
            foreach (var ptv in structureList.Where(s => s.Id.StartsWith("PTV", StringComparison.OrdinalIgnoreCase)))
            {
                ...
                foreach (var prefix in new[] { "CTV", "GTV" })
                {
                    ...
                    if (target != null && !IsStructureContained(target, ptv))
                    {
                        results.Add(CreateResult(
                            "ClinicalGoals.TargetContainment",
        ...
        private static void ValidateTargetStructureTypes(IEnumerable<Structure> structures, List<ValidationResult> results)
        {
            foreach (var structure in structures)
            {
                if (structure.Id.StartsWith("PTV", StringComparison.OrdinalIgnoreCase) &&
                    !string.Equals(structure.StructureType, "PTV", StringComparison.OrdinalIgnoreCase))
                {
                    results.Add(CreateResult(
                        "ClinicalGoals.StructureTypes",
                        $"Structure '{structure.Id}' should be of type PTV but is '{structure.StructureType}'.",
                        ValidationSeverity.Error));
                }

                if (structure.Id.StartsWith("CTV", StringComparison.OrdinalIgnoreCase) &&
                    !string.Equals(structure.StructureType, "CTV", StringComparison.OrdinalIgnoreCase))
                {
                    results.Add(CreateResult(
                        "ClinicalGoals.StructureTypes",
                        $"Structure '{structure.Id}' should be of type CTV but is '{structure.StructureType}'.",
                        ValidationSeverity.Error));
                }

                if (structure.Id.StartsWith("GTV", StringComparison.OrdinalIgnoreCase) &&
                    !string.Equals(structure.StructureType, "GTV", StringComparison.OrdinalIgnoreCase))
                {
                    results.Add(CreateResult(
                        "ClinicalGoals.StructureTypes",
                        $"Structure '{structure.Id}' should be of type GTV but is '{structure.StructureType}'.",
                        ValidationSeverity.Error));
                }
            }
        }

        private static bool IsStructureContained(Structure inner, Structure outer)
        {
            if (inner == null || outer == null || inner.IsEmpty || outer.IsEmpty)
                return true;

            var image = inner.StructureSet.Image;
            if (image == null)
                return true;
        ...
        private static bool StructuresOverlap(Structure a, Structure b)
        {
            if (a == null || b == null || a.IsEmpty || b.IsEmpty)
                return false;

            var image = a.StructureSet.Image;
            if (image == null)
                return false;
        ...
        private static string GetStructureId(object goal)
        {
            ...
            var structureObj = GetPropertyValue(goal, "Structure") as Structure;
        ...
        private static bool IsStructureExcluded(Structure structure)
        {
            ...
            return ExcludedBones.Contains(structure.Id);
        }
    }
